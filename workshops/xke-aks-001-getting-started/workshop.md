...
tags: kubernetes, azure, aks, keyvault, pvc, mesh, monitoring, scaling, keda, graphana, prometheus # Required. Tags for filtering and searching
---
...
> There is an environment set up in our subscription "Xpirit MPN sponsored BELGIUM". If you are using this you can skip below steps. You have rights using rg-shared-xke and rg-users-\<your Xpirit shortname>
...
> Before you proceed, please ensure you have access to an Azure subscription with the ability to create resources. If you do not have access to an Azure subscription, you can sign up for a [free account](https://azure.microsoft.com/free).
...
4. To check if the providers are registered run the following commands. It should look like "Registered"
  ```bash
  az provider register --namespace Microsoft.Quota --query "registrationState"
  az provider register --namespace Microsoft.Compute --query "registrationState"
  az provider register --namespace Microsoft.ContainerRegistry --query "registrationState"
  az provider register --namespace Microsoft.ContainerService --query "registrationState"
  az provider register --namespace Microsoft.Network --query "registrationState"
  az provider register --namespace Microsoft.ApiManagement --query "registrationState"
  az provider register --namespace Microsoft.Monitor --query "registrationState"
  az provider register --namespace Microsoft.AlertsManagement --query "registrationState"
  az provider register --namespace Microsoft.Dashboard --query "registrationState"
  az provider register --namespace Microsoft.App --query "registrationState"
  az provider register --namespace Microsoft.CloudShell --query "registrationState"
  ```

5. Run the following commands to ensure you have all the necessary features registered in your subscription.
  ```bash
...
6. To check if the features are registered run the following commands. It should look like "Registered"
  ```bash
  az feature show --namespace "Microsoft.ContainerService" --name "EnableWorkloadIdentityPreview" --query "properties.state"
  az feature show --namespace "Microsoft.ContainerService" --name "AKS-GitOps" --query "properties.state"
  az feature show --namespace "Microsoft.ContainerService" --name "AzureServiceMeshPreview" --query "properties.state"
  az feature show --namespace "Microsoft.ContainerService" --name "AKS-KedaPreview" --query "properties.state"
  az feature show --namespace "Microsoft.ContainerService" --name "AKS-PrometheusAddonPreview" --query "properties.state"
  ```
7. Clone this repo: https://github.com/sujithq/awesome-aks
8. Using your terminal, open the repo and navigate to the **2023-12-19-xke-aks-workshop** directory
9. Run the following command to create a **terraform.tfvars** file and populate it with the following content.
```terraform
cat <<EOF > terraform.tfvars
deployment_locations = [
  {
    users: ["<your Xpirit shortname>"]
    location = "westeurope"
...
EOF
```
10. Run the `terraform init` command
11. Run the `terraform plan -out tf.plan` command
11. Run the `terraform apply tf.plan` command
...
> Run the following command to set variables for your resource group and AKS cluster name. Don't forget to replace `<userName>` in the command below with the username you've been assigned.
...
```bash
kubectl config use-context aks-user-$USR
```
...
> When you run multiple containers in a Pod, this is known as a [sidecar pattern](https://learn.microsoft.com/en-us/azure/architecture/patterns/sidecar).
...
az account set -s <Take the id where name equals Xpirit MPN sponsored BELGIUM>
```

<div class="task" data-title="Task">

> Run the following command to set variables for your resource group and AKS cluster name and don't forget to replace `<userName>` in the command below with the username you've been assigned.
...
> Open a new terminal and make sure you are at the root of the repo then run the following command set variables for your resource group and AKS cluster names. Be sure to replace `<userName>` with your assigned user number.
...
To learn more about Kubernetes, check out the [Kubernetes Learning Path](https://azure.microsoft.com/resources/kubernetes-learning-path/) and be sure to check out the [AKS docs](https://docs.microsoft.com/azure/aks/). 

If you have any questions or feedback, please let me know in the comments below or reach out to me on Twitter [@SujithQ](https://twitter.com/SujithQ) or LinkedIn [/in/sujithquintelier](https://www.linkedin.com/in/sujithquintelier/)
